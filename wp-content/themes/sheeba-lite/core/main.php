...
if (!function_exists('sheeba_lite_customize_excerpt_more')) {
	...
	function sheeba_lite_customize_excerpt_more($excerpt) {
		...
		if ( sheeba_lite_is_single() ) :
			...
			return $content. '<a '. wp_kses($container, $allowed) . ' href="' . esc_url(get_permalink($post->ID)) . '" title="'.esc_attr__('Read More','sheeba-lite').'"> <span class="'.esc_attr($class).'">'.$button.'</span></a>';
