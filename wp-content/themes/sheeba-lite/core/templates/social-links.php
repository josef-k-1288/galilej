...
if (!function_exists('sheeba_lite_socials_links_function')) {

	function sheeba_lite_socials_links_function() {
		...
		foreach ( $socials as $name => $attrs) { 
			...
			if (sheeba_lite_setting('sheeba_lite_'.$name.'_button')): 

				$i++;
				
				$link = 	esc_url(sheeba_lite_setting('sheeba_lite_'.$name.'_button'), array( 'http', 'https', 'tel', 'skype', 'mailto' ) );
		...
		if ( sheeba_lite_setting('sheeba_lite_rss_button') == "on" ): 
		...
		if ( $i > 0 ) {
			echo '<div class="social-links-wrapper">' .  wp_kses($html, $allowed) . '</div>';
		}
		
	}
	
	add_action( 'sheeba_lite_socials_links', 'sheeba_lite_socials_links_function' );
